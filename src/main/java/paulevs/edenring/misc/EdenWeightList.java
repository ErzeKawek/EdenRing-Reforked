...
public class EdenWeightList<T> {
    ...
    public static <T> Codec<EdenWeightList<T>> listCodec(Codec<T> elementCodec, String fieldName, String elementName) {
        return RecordCodecBuilder.create(instance -> instance
                .group(
                        pairCodec(elementCodec, elementName).listOf()
                                .fieldOf(fieldName)
                                .forGetter(EdenWeightList::pairs)
                )
                .apply(instance, EdenWeightList::new)
        );
    }
    ...
    private EdenWeightList(List<Pair<Float, T>> pairs) {
        maxWeight = 0;
        for (var pair : pairs) {
    ...
    public EdenWeightList() {
    }

    public <R> EdenWeightList<R> map(Function<T, R> map) {
        List<Pair<Float, R>> pairs = new ArrayList<>(weights.size());
        for (int i = 0; i < weights.size(); i++) {
            pairs.add(new Pair<>(weights.get(i), map.apply(values.get(i))));
        }
        return new EdenWeightList<>(pairs);
    }

    public void addAll(EdenWeightList<T> other) {
    ...
    protected EdenWeightList<T> subList(int start, int end) {
        EdenWeightList<T> list = new EdenWeightList<T>();
